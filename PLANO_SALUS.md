...
4. **Vinculo Biologico como Padrão**: Assume-se vínculo biológico entre humanos da família por default (exceções registradas em comentário interno), simplificando a Ficha e protegendo a privacidade.
...
| Parentesco e Vínculo | **Árvore visual + Vínculo Biológico default** | A família é visualizada no `Arvore.md`. Humanos são biológicos por padrão; exceções ficam em comentário invisível no `META.md`. Ficha individual não expõe relação. |
...
├── .agents/skills/
│   ├── salus-onboarding/     ← entrevista inicial (cria perfis e árvore familiar)
...
└── Familia/
    ├── META.md               ← indice geral (membros, papel na família e metadados de vínculo ocultos)
    ├── Arvore.md             ← diagrama Mermaid da estrutura familiar
